...
lemma.freq.long <- lemma.freq %>%
  tidyr::pivot_longer(cols = c(Radiology, Orthopedic),
                      names_to = "comparison_specialty",
                      values_to = "x_proportion")


ggplot2::ggplot(lemma.freq.long, ggplot2::aes(x = x_proportion, 
                                              y = Surgery,
                                              color = abs(Surgery - x_proportion))) + 
  ggplot2::geom_abline(color = "gray40", lty = 2) +
  ggplot2::geom_jitter(alpha = 0.1, size = 2.5, width = 0.3, height = 0.3) +
  ggplot2::geom_text(ggplot2::aes(label = lemma), check_overlap = TRUE, vjust = 1.5) +
  ggplot2::scale_x_log10(labels = scales::percent_format()) + 
  ...
  ggplot2::facet_wrap(~comparison_specialty, ncol = 2) + 
  ggplot2::theme(legend.position = "none") +
...
And also let's do the other plotting:
